...
class RideHistoryViewController: UIViewController {
    ...
    var ary_PaymentHistoryInfo: [RideHistoryViewDatar] = []
    lazy var viewModel = {
...
extension RideHistoryViewController : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell:RideHistoryTableViewCell = tableView.dequeueReusableCell(withIdentifier: "RideHistoryTableViewCell", for: indexPath) as? RideHistoryTableViewCell else {return UITableViewCell()}
        var str_BookingType = ary_PaymentHistoryInfo[indexPath.row].booking_type ?? ""
        var str_CurrentRideDateTime = ""
        if str_BookingType == "1" {
            cell.btn_StopActionRef.isHidden = false
        } else {
            cell.btn_StopActionRef.isHidden = true
        }
        ...
        cell.btn_StopActionRef.isHidden = true
        cell.btn_StopActionRef.layer.cornerRadius = 5.0
        cell.btn_StopActionRef.layer.masksToBounds = true
        cell.btn_StopActionRef.tag = indexPath.row;
        cell.btn_StopActionRef.addTarget(self, action: #selector(stopsButtonClicked), for: .touchUpInside)
    ...
    @objc func stopsButtonClicked(sender: Any) {
