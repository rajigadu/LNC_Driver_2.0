        ...
        self.viewModel.requestForActivePartnerAPIServices(perams: ["driverid":"255"]) { success, model, error in
            if success, let userData = model {
                DispatchQueue.main.async { [self] in
                    ...
                    self.txt_PartnerNameref.text = userData.data?[0].partner_name ?? ""
                    self.txt_PartnerMailRef.text = userData.data?[0].partner_email ?? ""
                    self.txt_PartnerMobilenumberRef.text = userData.data?[0].partner_phone ?? ""
            ...
            if success, let UserData = model {
                DispatchQueue.main.async { [self] in
                    ...
                    if self.str_CheckingFromRide == "AcceptRideCase" {
                        ...
                        var str_ServiceType = UserData.data?[0].service_type ?? ""
