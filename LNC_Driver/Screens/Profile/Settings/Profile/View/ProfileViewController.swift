...
class ProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //First Name
        if let Str_FirstName = UserDefaults.standard.string(forKey: "DriverFirstName") as? String {
            self.txt_FirstNameRef.text = Str_FirstName
        }
        //Last Name
        if let Str_LastName = UserDefaults.standard.string(forKey: "DriverLasttName") as? String {
            self.txt_LastNameRef.text = Str_LastName
         }
        //Mobile Number
        if let Str_Mobilenumber = UserDefaults.standard.string(forKey: "DriverMobilenumber") as? String {
            self.txt_MobilenumberRef.text = Str_Mobilenumber
         }
        //User Image
        self.imageview_ProfilepicRef.image = UIImage(named: "personIcon")
        if let Str_UserImage = UserDefaults.standard.string(forKey: "DriverProfilepic") as? String {
            isProfilePicTaken = true
            self.imageview_ProfilepicRef.sd_setImage(with: URL(string: Str_UserImage), placeholderImage: UIImage(named: "personIcon"))
